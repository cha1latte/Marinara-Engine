...
Open **Settings → Extensions** and click the 🔔 bell icon on this extension's card. A popover opens next to the icon where you can:
...
- **Close** — dismiss the popover (clicking outside it also closes).

To temporarily disable the override and bring back the default ping, just toggle the extension off via its card on/off switch — no separate enabled flag inside the popover.

All settings are saved per-browser via `localStorage` and persist across reloads.

### File size

Audio is stored inline in `localStorage`, which has a ~5 MB per-origin cap shared across all extensions. Keep clips under ~1 MB (a few seconds of mp3 is plenty). If a file is too large the popover will say so — pick a shorter or lower-bitrate clip.

### Setting values without the popover

If you'd rather skip the popover — useful for sharing a config or scripting setup — open DevTools and run:

```js
// Sound must be a data URL (data:audio/...;base64,...). The popover does the
...
- If no sound is uploaded, the patch falls through to the original `start()` and the default ping plays unchanged.
- The bell icon and popover are injected into the extension card via a 500 ms polling loop that matches the extension by name and inserts a button before the existing trash icon. This pattern is borrowed from [Decidetto's Accent Color Changer](https://github.com/Decidetto/ME-accent-color-changer) extension.
- On unload (extension toggled off or removed), the prototype patch is restored and the bell icon is cleaned up automatically by Marinara's extension lifecycle.

## Persisted localStorage keys

| Key | Type | Default | Notes |
|---|---|---|---|
| `marinara-cns-sound` | data URL string | `""` | Base64-encoded audio, written by the popover's file picker |
| `marinara-cns-name` | string | `""` | Original filename, shown in the popover |
...
- **Card selector fragility.** The bell icon is injected by matching `span.font-medium.truncate` containing the extension name and walking to the nearest `[class*="rounded-lg"]` ancestor. If Marinara restyles the Extensions panel with different class names, the icon will silently stop appearing until the selectors are updated.
...
The extension-card bell icon + popover injection pattern is adapted from [Decidetto's Accent Color Changer](https://github.com/Decidetto/ME-accent-color-changer). Thanks Decidetto!
